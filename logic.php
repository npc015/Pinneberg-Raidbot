...
function keys_vote($raid)
{
	...
	// Get minute and convert modulo raidslot
        ...
        // Add time keys.
        $buttons_time = inline_key_array($keys_time, 3);
        ...
        $keys_default = explode(',', 'extra,time,pokemon,status');
        ...
        if(count($keys_UI_config) == count($keys_default) && count(array_diff($keys_UI_config, $keys_default)) == 0){
            ...
            // Default keys order
            $keys = array_merge($buttons_time,$buttons_extra,$buttons_pokemon,$buttons_status);
        }
    }
...
function show_raid_poll($raid)
{
    ...
    // Display creator.
    if (!empty($raid['ingame'])) {	
    $msg .= ($raid['user_id'] && $raid['ingame']) ? (CR . getPublicTranslation('created_by') . ': <a href="tg://user?id=' . $raid['user_id'] . '">' . htmlspecialchars($raid['ingame']) . '</a>') : '';	
    }	
    else
...
//______________________________________
// Custom: alarm and send alarm function
function alarm($raid,$user,$action,$info = '')
{	
	// Name des User der etwas ändert abfragen
	$request = my_query("SELECT * FROM users WHERE user_id = {$user}");
	$answer_quests = $request->fetch_assoc();
        if (!empty($answer_quests['ingame'])) {
        $username = $answer_quests['ingame'];
        } else {
	$username = $answer_quests['name'];
          }
	

	// Arenenname vom Raid abfragen
	$request = my_query("SELECT * FROM raids as r left join gyms as g on r.gym_id = g.id WHERE r.id = {$raid}");
	// my_query("SELECT * FROM raids WHERE id = {$raid}");
	$answer_quests = $request->fetch_assoc();
	$gymname = $answer_quests['gym_name'];
	
	// Wenn ein weiterer Gast eingetragen wird
	if($action == "extra")
	{
		// Englisch in Deutsch umwandeln
		$farb_alt = array('mystic', 'valor', 'instinct');
		$farb_neu = array ('blauen', 'roten', 'gelben');
		$farbe = str_replace($farb_alt, $farb_neu, $info);
		
		// Nachricht senden
		sendalarm("<b>".$username."</b> bringt noch einen weiteren <b>".$farbe." Trainer</b> zum Raid an der Arena <b>".$gymname."</b> mit.",$raid);
	}
	else if($action == "status")
	{	
		// Wenn ein Trainer seinen Status ändert (zu spät oder Absage)
		if($info == 'late')
			sendalarm("<b>".$username."</b> kommt 5 Minuten später zum Raid an der Arena <b>".$gymname."</b>.",$raid);
		else if($info == 'cancel')
			sendalarm("<b>".$username."</b> hat seine Teilnahme am Raid an der Arena <b>".$gymname." abgesagt.</b>.",$raid);
	}
	else if($action == "pok_teil")
    { 
    if($info == 0)
    {
      sendalarm("<b>".$username."</b> möchte am Raid an der Arena <b>".$gymname."</b> bei jedem Raidboss teilnehemn.",$raid);
    }
    else
    {
      $poke_name = get_local_pokemon_name($info);
      if($poke_name != 'Raid Ei')
        sendalarm("<b>".$username."</b> möchte am Raid an der Arena <b>".$gymname."</b> beim Raidboss <b>".$poke_name."</b> teilnehmen.",$raid);
    }
	}	
	else if($action == "neu_teil")
	{	
			// Eine gewöhnliche Teilnahme
			sendalarm("<b>".$username."</b> möchte am Raid an der Arena <b>".$gymname."</b> um <b>".date('H:i', strtotime($info)+3600)."</b> Uhr teilnehmen.",$raid);
	}
	else if($action == "aend_teil")
	{	
			// Änderung der Uhrzeit
			
			//sendalarm("<b>".$username."</b> hat die Uhrzeit für die Raid-Teilnahme an der Arena <b>".$gymname."</b> auf <b>".date('H:i', $info)."</b> Uhr geändert.",$raid);
			sendalarm("<b>".$username."</b> hat die Uhrzeit für die Raid-Teilnahme an der Arena <b>".$gymname."</b> auf <b>".date('H:i', strtotime($info)+3600)."</b> Uhr geändert.",$raid);
	}
	else if($action == "extra_alleine")
	{ 
	sendalarm("<b>".$username."</b> kommt doch lieber alleine zum Raid an der Arena <b>".$gymname."</b>.",$raid);
	}
}

// Funktion zum Versenden des Alarms
//______________________________________
// Custom: alarm and send alarm function
function sendalarm($text, $raid)

{	
	// Ermittelt alle Trainer, die für dieses Raid benachrichtigt werden sollen und sendet jedem eine Nachricht
